...
Begin
For i := $00 to $FF do Begin					// fill the table
	y := 0;
	For x := 0 to 7 do  y := y OR				// go through all bits
		(((i SHR x) AND Bit0) SHL (7 - x));		// get bit, insert it at new pos.
	Table[i] := y;
End;
i := 0;
