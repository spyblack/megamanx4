...
var
  ...
  number_disk : uLong; { number of the current dist, used for spanning ZIP,
...
Var	uDate	:	uLong;
Begin
uDate		:= ((ulDosDate	SHR 16		)		)	;
ptm.tm_mday	:= ((uDate	AND $001F	)		)	;
ptm.tm_mon	:= ((uDate	AND $01E0	) div $0020	) + 1	;
ptm.tm_year	:= ((uDate	AND $FE00	) div $0200	) + 1980;
ptm.tm_hour	:= ((ulDosDate	AND $F800	) div $0800	)	;
ptm.tm_min	:= ((ulDosDate	AND $07E0	) div $0020	)	;
ptm.tm_sec	:= ((ulDosDate	AND $001F	) * 2	   	)	;
End;
...
function unzlocal_GetCurrentFileInfoInternal (
  ...
  uSizeRead := file_info.size_file_comment;
    ...
    err := UNZ_ERRNO;
      ...
      {Inc(lSeek, file_info.size_file_comment - uSizeRead)};  // EDIT: never used
    end
    else
      {Inc(lSeek, file_info.size_file_comment)};  // EDIT: never used
...
begin
  ...
  if ((s^.cur_file_info.compression_method<>0) and
      (s^.cur_file_info.compression_method<>Z_DEFLATED)) then
    {err := UNZ_BADZIPFILE};  // EDIT: never used
